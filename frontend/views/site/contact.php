...
<div class="site-contact">
    ...
    <div class="row mt-3 ">
        ...
        <div class="col-12 col-lg-6 order-sm-first order-xs-first my-3">
            <h3 class="fw-bold mb-4 text-center"><span class=" border-3 border-dark border-bottom ">GỬI THẮC MẮC CỦA BẠN</span>
            </h3>
            <div class="hr"></div>
            <?php $form = ActiveForm::begin(['id' => 'contact-form']); ?>

            <?= $form->field($model, 'name')->textInput(['autofocus' => true])->label('Họ và Tên:') ?>

            <div class="row">
                <div class="col-md-6 my-3">
                    <?= $form->field($model, 'email')->label('Địa chỉ mail:') ?>
                </div>
                <div class="col-md-6 my-3">
                    <?= $form->field($model, 'tel')->label('Số điện thoại:') ?>
                </div>
            </div>

            <?= $form->field($model, 'content')->textarea(['rows' => 6])->label('Nội dung liên hệ:')?>
