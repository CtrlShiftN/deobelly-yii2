                ...
                <div class="border-radius bg-light">
                    <div class="mx-4">
                        ...
                        <?= $form->field($model,'tel')
                            ->textInput(['type' => 'number','class' => 'rounded-pill form-control mb-2'])
                            ->label(Yii::t('app','Enter phone number')) ?>
                        <?= $form->field($model,'email')
                            ->textInput(['type' => 'email','class' => 'rounded-pill form-control mb-2'])
                            ->label(Yii::t('app','Enter email')) ?>
                        <?= $form->field($model,'password')
                            ->textInput(['type' => 'password','class' => 'rounded-pill form-control mb-2'])
                            ->label(Yii::t('app','Enter password')) ?>
                        <?= $form->field($model,'password_confirm')
                            ->textInput(['type' => 'password','class' => 'rounded-pill form-control mb-2'])
                            ->label(Yii::t('app','Enter first and last name')) ?>
                        <?= Html::submitButton(Yii::t('app','Signup'), ['class' => 'btn btn-primary rounded-pill text-light w-100 mt-3', 'name' => 'signup-button']) ?>
                    </div>
